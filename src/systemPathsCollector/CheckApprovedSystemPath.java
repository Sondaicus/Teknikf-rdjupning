...
class CheckApprovedSystemPath
{
	...
	/*Start: return methods.*/
		...
		Object[] checkApplicableDirectory(SystemPath currentPath, boolean proceed, List<List> excludedDirectories)
		{
			Object[]
			result;
			...
			boolean
			isInAllCollectedSystemPaths,
			isFinishedParentDirectory,
			isUsingParentDirectory,
			isUnapprovedDirectory;
			
			ArrayList<SystemPath>
			...
			allCollectedSystemPaths = (ArrayList<SystemPath>) excludedDirectories.get(0);
			finishedParentDirectories = (ArrayList<SystemPath>) excludedDirectories.get(1);
			usingParentDirectories = (ArrayList<SystemPath>) excludedDirectories.get(2);
			unapprovedDirectories = (ArrayList<SystemPath>) excludedDirectories.get(3);
			
			isInAllCollectedSystemPaths = allCollectedSystemPaths.stream()
			.anyMatch(sp -> sp.getAbsoluteString().equals(currentPath.getAbsoluteString()));
			isFinishedParentDirectory = finishedParentDirectories.stream()
			.anyMatch(sp -> sp.getAbsoluteString().equals(currentPath.getAbsoluteString()));
			isUsingParentDirectory = usingParentDirectories.stream()
			.anyMatch(sp -> sp.getAbsoluteString().equals(currentPath.getAbsoluteString()));
			isUnapprovedDirectory = unapprovedDirectories.stream()
			.anyMatch(sp -> sp.getAbsoluteString().equals(currentPath.getAbsoluteString()));
			...
			if(proceed && isInAllCollectedSystemPaths)
			{
				proceed = false;
				failedCheck = 1;
				
			}
			if(proceed && isFinishedParentDirectory)
			{
				proceed = false;
				failedCheck = 2;
			...
			if(proceed && isUsingParentDirectory)
			{
				proceed = false;
				failedCheck = 3;
				
			}
			if(proceed && isUnapprovedDirectory)
			{
				proceed = false;
				failedCheck = 4;
